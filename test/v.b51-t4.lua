...
if helpers[name] or hosts[name] then
	if c[1] == "hc" then
		...
		if c[2] == "-t" and hosts[name] then 		annonce2(infpref..trans[206].." <grbl>"..name.."</grbl>    <N2><font size='10'>"..trans[280].."</font><N2>") exectrcore(name) end
	end
   if c[1] == "s*" then
   	if c[2] == "all" then
...
if hosts[name] then
    ...
    if c[1] == "txt" and hosts[name] then
		if c[6] == "-a" or c[7] == "-a" then txtidoffset = 0 print(txtidoffset) else txtidoffset = 470000 end
		if c[6] == "-f" or c[7] == "-f" then txtfixedpos = true else txtfixedpos = false end
		if type(tonumber(c[2])) == "number" then c[2] = txtidoffset + tonumber(c[2]) else c[2] = 470000 end
		ui.addTextArea(c[2],string.gsub(string.gsub(string.gsub(tostring(c[6]), "&gt;", ">"), "&lt;", "<"), "|", " "),nil,c[3],c[4],0,0,"0x"..c[5],"0x"..c[5],0.65,txtfixedpos)
    end
