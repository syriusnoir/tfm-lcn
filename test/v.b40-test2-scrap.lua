...
function eventNewPlayer(name)
	...
	if hosts[name] or contestants[name] then
		ui.addTextArea(8880, "<J>"..getTranslation(name)[100].."</J> <V>"..version.."</V>\n\n<J><font size='14'><VP><font face='Wingdings 3'>u</font></VP> "..getTranslation(name)[101].."</font></J>\n\n<V>"..getTranslation(playerName)[102].."\n"..getTranslation(name)[103].."</V>\n\n<J><font size='14'><VP><font face='Wingdings 3'>u</font></VP> "..getTranslation(name)[104].."</font></J>\n\n<V>"..PLANNEDCONTEST.."</V>", name, 440, 80, 340, 330, 0x000000, 0x000000, 1, true)
		print("<PS>["..version.."] <S>"..name.." "..getTranslation(name)[200])
		tfm.exec.setPlayerScore(name,0)
	elseif not contestants[name] or not hosts[name] then
	--	ui.addTextArea(100, getTranslation(playerName)[80], name, -600, -200, 2400, 1200, 0x000001, 0x000001, 1, true)
	--	ui.addTextArea(101, "<ROSE><font size='40'><p align='center'>"..getTranslation(playerName)[201].."</p></font>\n<A:ACTIVE><font size='24'><p align='center'>"..getTranslation(playerName)[202].."</p></font>", name, 0, 20, 800, 384, 0x324650, 0x000000, 0, true)
...
for name,player in pairs(tfm.get.room.playerList) do
	...
	eventNewPlayer(name)
elseif not contestants[name] or not hosts[name] then
	ui.addTextArea(100, getTranslation(name)[80], name, -600, -200, 2400, 1200, 0x000001, 0x000001, 1, true)
	ui.addTextArea(101, "<ROSE><font size='40'><p align='center'>"..getTranslation(name)[201].."</p></font>\n<A:ACTIVE><font size='24'><p align='center'>"..getTranslation(name)[202].."</p></font>", name, 0, 20, 800, 384, 0x324650, 0x000000, 0, true)
	tfm.exec.killPlayer(name)
	tfm.exec.setPlayerScore(name,-32767)
end
end
...
if cmd:sub(1,1) == "i" then ui.addTextArea(8999, "<font size='16'><warn><a href='event:user.cls'>X</a></warn> <rose><b>"..getTranslation(name)[203].."</b></rose></font>\n______________\n\n<font size='14'><warn>"..getTranslation(name)[204].."</warn></font>\n<font size='12'><purp><li><a href='event:user.nogui'>"..getTranslation(name)[205].."</a>\n", name, 325, 100, 150, 200, 0x000001, 0x000000, 0.9, true) end
-- 'all' komandas
...
if cmd=="hc" and hosts[name] or helpers[name] and cmd=="hc" then 
		hostcore(name)
		annonce2(infpref..getTranslation(name)[206].." <V>"..name.."</V></T>     <N2><font size='10'>"..getTranslation(name)[207].."</font><N2>")
end
...
-- Augsta līmeņa komandas
    ...
    if c[1] == "tfm" and hosts[name] then
        tfm.exec.giveGetTranslation(name)formations(c[2])
    elseif c[1] == "tfm" then
        error0x1(name)
    end
    if c[1] == "defcol" and hosts[name] then
			setColorTag(c[2], c[3])
			annonce2(infpref..""..name.." "..getTranslation(name)[208].." <"..c[2]..">"..c[2].."</"..c[2]..">.")
    elseif c[1] == "defcol" then
        error0x1(name)
    end
    if c[1] == "tfm*" and hosts[name] then
        tfm.exec.giveGetTranslation(name)formations(c[2],false)
    elseif c[1] == "tfm*" then
        error0x1(name)
    end
    if c[1] == "d" and hosts[name] then
			ui.addTextArea(101, "<R><font size='40'>"..getTranslation(name)[209].."</font>\n<font size='16'><A:ACTIVE>"..getTranslation(name)[210].."\n"..getTranslation(name)[211].."</J></font></R>", c[2], 100, 150, 600, 100, 0x000001, 0x000001, 1, true) 
			tfm.exec.killPlayer(c[2])
			tfm.exec.setPlayerScore(c[2],-32768,false) 
			ui.updateTextArea(32,"<D>• "..c[2].." "..getTranslation(name)[212],nil)
			print("<D>• "..c[2].." "..getTranslation(name)[212])			
    elseif c[1] == "d" then
        error0x1(name)
    end
    if c[1] == "role" and hosts[name] then
		    if c[2] == "+" then  
			if c[3] == "host" then hosts[c[4]] = true annonce2(infpref..c[4].." "..getTranslation(name)[213]) end
			if c[3] == "helper" then helpers[c[4]] = true annonce2(infpref..c[4].." "..getTranslation(name)[214]) end end
		    if c[2] == "-" then  
			if c[3] == "host" then hosts[c[4]] = false annonce2(infpref..c[4].." "..getTranslation(name)[215]) end
			if c[3] == "helper" then helpers[c[4]] = false annonce2(infpref..c[4].." "..getTranslation(name)[216]) end end
...
ui.updateTextArea(32,"<warn><b>• ["..name.."]</b> "..getTranslation(name)[217],nil)
end
function error0x2(name,cmd)
ui.updateTextArea(32,"<warn><b>• ["..name.."]</b> "..getTranslation(name)[218],nil)
end
...
tfm.exec.newGame(tfm.get.room.currentMap) annonce2(infpref..n.." "..getTranslation(n)[219]) 
...
function eventMouse(n, x, y)
  ...
  if shifting[n] then
    ui.updateTextArea(32,"<BV>• "..n .." "..getTranslation(n)[220].." X:" .. x .. ", Y:" .. y,nil)
    print("<BV>• "..n .." "..getTranslation(n)[220].." X:" .. x .. ", Y:" .. y)
  end
end
...
-- Prefiksi
syscore = "<ROSE><b>"..getTranslation()[221].."</b>" 
...
ui.addTextArea(65, "<PS><b>[•]</b> "..getTranslation(name)[222], name, 100, 25, 601, 20, 0x324650, 0x000000, 0, true)
ui.addTextArea(0, "<ROSE><p align='center'><font size='20'>"..title.."</font></p>", name, 100, -35, 600, 30, 0x324650, 0x000000, 0.5, true)
ui.addTextArea(64, "<PT><b>[•]</b> <T>"..getTranslation(name)[223].." <V>"..version.."</V> / "..getTranslation(name)[224].." <V>"..OWNER_ID.."</V>", name, 100, 40, 601, 20, 0x324650, 0x000000, 0, true)
end
...
print("<BV>• "..name.." "..getTranslation(name)[225])
ui.updateTextArea(0,"<p align='center'><font size='20'><ROSE>"..name.." "..getTranslation(name)[226].."</font></p>",nil)
end
...
ui.updateTextArea(0,"<VP><p align='center'><font size='20'>"..getTranslation(name)[227].."</font></p>", nil)
survRound()
tfm.exec.setUIShamanName("<VP>La Cros Nostra")
ui.updateTextArea(32,syscore.." "..getTranslation(name)[228],nil)
end
...
if survrounds ~= 10 then
ui.updateTextArea(0,"<ROSE>"..survrounds..getTranslation()[40].." "..getTranslation()[229],nil)
elseif survrounds == 10 then
ui.updateTextArea(0,"<ROSE>"..getTranslation()[230],nil)
end
end
function winnerFunction(winner)
ui.updateTextArea(0,"<p align='center'><font size='20'><ROSE>"..winner.." "..getTranslation()[231].."</font></p>",nil)
ui.updateTextArea(32,"<D>• "..survrounds..getTranslation(playerName)[40].." "..getTranslation()[232].." "..winner.." "..getTranslation()[231],nil)
print('<D>• '..survrounds..getTranslation()[40]..' '..getTranslation()[233]..' '..winner..' '..getTranslation()[231])
...
ui.addTextArea(0,"<VP><p align='center'><font size='40'>"..getTranslation()[234].."</font></p>", nil, 25, 25, 750, 60, 0x324650, 0x000000, 0.1, true)
nextMap()
end
function nextMap()
...
function hostinterfaces(name)
	if hosts[name] then
		...
		annonce2(infpref..getTranslation(name)[235].." (<V>"..name.."</V>)") 
	end
end
...
function eventPlayerWon(playerName, timeElapsed, timeElapsedSinceRespawn)
	if SETUP.challengemode == "pkg" then
		if first == 0 then
			...
			ui.updateTextArea(32,"<D>• "..pkg.round..getTranslation(playerName)[40].." "..getTranslation(playerName)[233].." "..playerName.." "..getTranslation(playerName)[231],nil)
			print("<D>• "..pkg.round..getTranslation(playerName)[40].." "..getTranslation(playerName)[233].." "..playerName.." "..getTranslation(playerName)[231])	
...
ui.addTextArea(101, "<R><font size='40'>"..getTranslation(playerName)[237].."</font>\n<font size='16'>"..getTranslation(playerName)[238].." <J>"..OWNER_ID.."</J>.</font></R>", nil, 100, 150, 600, 100, 0x324650, 0x000000, 0.1, true) end
...
function eventNewGame(name)
	...
	ui.updateTextArea(0,"<ROSE><p align='center'><font size='20'>"..pkg.round..getTranslation()[40].." "..getTranslation()[229].."</font></p>", nil)
...
function eventTextAreaCallback(textAreaID, playerName, callback,id,other)
	if callback=="spectate" then 
		...
		ui.updateTextArea(101,"<R><font size='40'><p align='center'>"..getTranslation(playerName)[239].."</p></font>\n<N2><font size='24'><p align='center'>"..getTranslation(playerName)[211].."</p></font>",playerName)
	end
	...
	if callback=="init_hostcore" and hosts[playerName] then 
		hostcore(playerName)
		print(syscore.." "..getTranslation(playerName)[206].." (<V>"..playerName.."</V>)")
		ui.updateTextArea(65,syscore.." "..getTranslation(playerName)[206].." (<V>"..playerName.."</V>)",nil)
	end
