...
SETUP = {debug=true,challengemode="build",autoplay = false,autorespawn = false,lang="lv"} 
...
if helpers[name] or hosts[name] then
	if c[1] == "hc" then
		if c[2] ~="-b" then hostcore(name) userData[name].hc = true end
		if c[2] ~= "-t" and c[2] ~= "-b" then annonce2(infpref..trans[206].." <grbl>"..name.."</grbl></lbrp>     <N2><font size='10'>"..trans[207].."</font><N2>") end
...
function hostinterfaces(name)
	if hosts[name] or helpers[name] then
		ui.addTextArea(450, hidata[histate], name, hipos[userData[name].hipstate].x, hipos[userData[name].hipstate].y, 20, 165, 0x000022, 0x000022, 0.65, true)
		if SETUP.challengemode == "build" then
			ui.addTextArea(451, "<font size='14'><p align='center'><a href='event:toggleborder'><bv><font face='Wingdings 3'>F</font></bv></a>\n<a href='event:togglesailwater'><bv><font face='Wingdings'>h</font></bv></a>", name, hipos[userData[name].hipstate].x, hipos[userData[name].hipstate].y + 185, 20, 38, 0x000022, 0x000022, 0.65, true)
		end
