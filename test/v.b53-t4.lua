...
validcmds = {
	...
	helpers={["bc"]=true,["meep"]=true,["f"]=true,["f*"]=true,["n"]=true,["n*"]=true,["aie"]=true,["aie*"]=true,["ce"]=true,["ce*"]=true,["clear"]=true,["menu"]=true,["i"]=true,["hc"]=true,["notepad"]=true,["r"]=true,["r"]=true,["s"]=true,["s*"]=true,["size"]=true,["smode"]=true,["change"]=true,["gscale"]=true,["wscale"]=true},
	hosts={["bc*"]=true,["end"]=true,["end*"]=true,["kill"]=true,["test"]=true,["toggle"]=true,["sname"]=true,["uc"]=true,["uc*"]=true,["hc*"]=true,["next"]=true,["c"]=true,["t"]=true,["spawn*"]=true,["r*"]=true,["draw"]=true,["draw*"]=true,["np"]=true,["np*"]=true,["g"]=true,["g*"]=true,["barrier"]=true,["col"]=true,["sp"]=true,["score"]=true,["add"]=true,["link"]=true,["link*"]=true,["tfm"]=true,["defcol"]=true,["startfrom"]=true,["tfm*"]=true,["d"]=true,["d*"]=true,["cd"]=true,["userdata"]=true,["map"]=true,["defcol"]=true,["tr"]=true,["debug"]=true,["role"]=true,["p"]=true,["f1"]=true,["f2"]=true,["f3"]=true,["fa"]=true,["fs"]=true,["fieldui"]=true,["fh"]=true,["play"]=true,["msg"]=true,["txt"]=true,["settp"]=true,["action"]=true,["mp"]=true,["set"]=true,["setfreq"]=true,["gset"]=true,["wset"]=true,["iter"]=true}
}
...
if hosts[name] then
	...
	if c[1] == "iter" and hosts[name] then
		ui.addPopup(398,2,"<p align='center'><font size='16' color='#84b7d1'><b>"..trans[307].."</b></font>",name,440,startmenuy,340,true)
    ...
    if c[1] == "set" and hosts[name] then
			...
			SETUP[c[2]]=c[3]		
			annonce2(infpref..c[2].." = "..tostring(c[3]))
    end
	if c[1] == "setfreq" then
		if SETUP.challengemode=="mono" then
...
elseif key==192 then apoing[n] = down
  ...
  if debing[n] and SETUP.debug==true then
		...
		if ctrling[n] then deb(nil,"store") end
  end
end
...
function eventTextAreaCallback(textAreaID, playerName, callback)
	...
	if callback=="init_hostcore" and hosts[playerName] then
 	...
 	if callback=="ptag" then
		...
		elseif shifting[playerName] or #Ptag>120 then
			...
			ui.addPopup(501, 0, "<font face='Lucida Console,Menlo' color='#b3b3ff'>&lt;P<br>"..Ptag.."<br>/&gt;</font>", playerName, 250, 200, 300, true)
			trainingcore(playerName,"panel:ptag")
		end
	end
	if callback=="ptag_edit" then
		local Ptag = string.match(tfm.get.room.xmlMapInfo.xml, "<P (.-)/>") or ""
		ui.addPopup(501, 2, "<font face='Lucida Console,Menlo' color='#b3b3ff'><font color='#6C77C1'>&lt;P</font><br>"..Ptag.."<br><font color='#6C77C1'>/&gt;</font></font>", playerName, 250, 200, 300, true)
	end
	if callback=="help" then
		...
		fieldui("notepad",playerName,"",{field1="<V>"..table.concat(commandList.contestants," <BL>|</BL> ").."</V>\n\n<FC>"..table.concat(commandList.helpers," <BL>|</BL> ").."</FC>\n\n<ROSE>"..table.concat(commandList.hosts," <BL>|</BL> ").."</ROSE>",actions="<ROSE><a href='event:rmfieldui'>"..trans[242].."</a></ROSE>"})
		commandList = {}
		trainingcore(playerName,"hostcore:help")
	end
...
function eventPopupAnswer(popupID, name, answer)
	...
	if popupID==501 and hosts[name] then 
		...
		annonce2(infpref..name.." "..trans[219])
