...
SETUP = {debug=true,elevatedmode=true,challengemode="build",title="lacrosnostra",autorespawn = false,lang="lv"} 
...
if hosts[name] then
    ...
    if c[1] == "dump" then
		if c[3] == nil then c[3] = "dump" end
		if c[3]:sub(1,3)=="-t=" then if c[3]:sub(4)=="all" then dmptarg=nil else dmptarg = c[3]:sub(4) or name end else dmptarg = name end 
		if SETUP.elevatedmode==true then
			if c[2] == "errlog" then tfm.exec.chatMessage(errlog,dmptarg)
			elseif c[2] == "cmdlog" then tfm.exec.chatMessage(cmdlog,dmptarg)
			elseif c[2] == "actionlog" or "tclog" then tfm.exec.chatMessage(tclog,dmptarg) end
		else
		end
    end
    if c[1] == "np" then
