...
SETUP = {debug=true,challengemode="survival",title="lacrosnostra",autorespawn = false,lang="lv",theme="modern"} 
...
function deb(name,mode,arg1,arg2)
	...
	if SETUP.debug==true then 
		ui.addTextArea(200, "<PT><font face='Lucida Console,Monaco'>#LCN "..version.."\nDebug mode", name, 5, 25, nil, nil, 0x000001, 0x000001, 0.05, true)
		if mode=="remove" then ui.removeTextArea(200,name) end
...
elseif key==192 then apoing[n] = down
  ...
  if debing[n] and SETUP.debug==true then
		if ling[n] then deb(n,"debdisplay") end
		if eing[n] then deb(n,"remove") end
  end
