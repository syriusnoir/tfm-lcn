...
userData = {} requests = {} etc = {version=0.17,common = {C = "Nickname color",M = "Mouse color",N = "Nickname change"},cmd={C = "/colornick",M = "/colormouse",N = "/changenick"},size={[1]=0.1,[2]=0.5,[3]=1,[4]=1.5,[5]=2,[6]=5},sizeCommon={[1]="0.1 (minimum)",[2]="0.5 (small)",[3]="1 (default)",[4]="1.5 (bigger)",[5]="2 (big)",[6]="5 (maximum)"},features={[1]=3,[2]=5,[3]=7,[4]=8},ftdisp={[1]="Username change requests",[2]="FFA",[3]="Size change",[4]="Greeting message"},fte={[1]="",[2]="",[3]="",[4]=""}}
...
function debugkit(cmd,name)
	...
	elseif c[1]=="del" then
		if c[2] and c[3] then 
			if tonumber(c[2])<tonumber(c[3]) then
				if tonumber(c[3])>#requests then
					return "fail, invalid range"
				else
					for i=c[2],c[3] do
						requests[i]=requests[c[3]-2+i]
					end
					for i=#requests-(c[3]-c[2]),#requests do 
						requests[i]=nil
					end
				end
			else
			...
			return string.format("deleted requests in range [%s,%s] successfully",c[2],c[3])
		else
			return "fail, 2nd or 3rd argument not specified"
		end
	else 
