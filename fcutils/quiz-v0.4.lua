...
function eventLoop(elapsed,rmng)
	if etc.time == 1 then etc.time = etc.responsetime*2 end
	etc.time = etc.time - 1
...
function eventChatCommand(name,cmd)
	...
	if c[1]=="rt" then 
		if tonumber(c[2])~=nil then
			etc.responsetime = tonumber(c[2]) tfm.exec.chatMessage("<font color='#ff0000'>[•]</font> <N>The response time has been changed to "..c[2].."s.",name) 
		else
			tfm.exec.chatMessage("<font color='#ff0000'>[•]</font> <N>Argument error.",name)
		end
	end
	if c[1]=="hl" then 
		if c[2]~=nil then
			tfm.exec.chatMessage("<font color='#ff0000'>• "..cmd:sub(4).."</font>",name)
		end
	end
end
